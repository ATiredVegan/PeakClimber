...
def update_graph(start,end,hplc_df,sample_name,ax,canvas,window): 
    ...
    peaks = tk.Button(master=prominenceheight, text="Identify Peaks", command=lambda:[find_peaks(x,z,start,end,hplc_df,sample_name,hei.get(),prom.get(),ax,canvas,window),clear(prominenceheight)])
    peaks.grid(row=0,column=4)
    print(window.grid_slaves())

    
def find_peaks(x,z,start,end,hplc_df,sample_name,height,prominence,ax,canvas,window):
    height=float(height)
    ...
    peaks.grid(row=0,column=4)
    back = tk.Button(master=prominenceheight, text="Back", command=lambda: [clear(prominenceheight), clear(onward), clear(settings), update_graph(start, end, hplc_df, sample_name, ax, canvas, window)])
    Hovertip(back,"Return to previoius screen")
    back.grid(row=0,column=5)
